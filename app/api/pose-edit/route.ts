...
export async function POST(req: Request) {
  try {
    ...
    if (!apiKey) {
      ...
      }, { status: 500, headers: { "x-inova-pose-route": ROUTE_VERSION } });
    }
    ...
    if (!imageUrl || !pose) {
      return NextResponse.json({ error: "imageUrl dan pose wajib diisi." }, { status: 400, headers: { "x-inova-pose-route": ROUTE_VERSION } });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: `Gagal mengambil foto sumber: ${error instanceof Error ? error.message : "network error"}` }, { status: 502, headers: { "x-inova-pose-route": ROUTE_VERSION } });
    }
    if (!sourceResponse.ok) {
      return NextResponse.json({ error: `Gagal mengambil foto sumber dari Cloudinary (HTTP ${sourceResponse.status}).` }, { status: 502, headers: { "x-inova-pose-route": ROUTE_VERSION } });
    }
    ...
    if (!response.ok) {
      ...
      }, { status: response.status, headers: { "x-inova-pose-route": ROUTE_VERSION } });
    }
    ...
    return NextResponse.json({ imageUrl: `data:image/png;base64,${b64}`, diagnostics: { requestId, model, routeVersion: ROUTE_VERSION } }, { headers: { "x-inova-pose-route": ROUTE_VERSION } });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Terjadi kesalahan pada image edit.", diagnostics: { routeVersion: ROUTE_VERSION } }, { status: 500, headers: { "x-inova-pose-route": ROUTE_VERSION } });
  }
}
